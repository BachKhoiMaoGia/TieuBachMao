...
Chạy: python3 crawl_shopee.py
Sau đó: git add products_shopee.json shopee.html && git commit -m "Update Shopee backup" && git push origin main
"""
...
if __name__ == "__main__":
    ...
    # 4. Save products_shopee.json
    output = {
        "groups": [{"id": g["groupId"], "name": g["groupName"], "count": int(g["totalCount"])} for g in groups],
        "products": categorized,
        "total": len(all_items)
    }
    products_path = os.path.join(DIR, "products_shopee.json")
    with open(products_path, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"\n💾 Saved products_shopee.json")
    ...
    print(f"\n👉 Tiếp theo chạy:")
    print(f"   git add products_shopee.json shopee.html && git commit -m 'Update Shopee backup' && git push origin main")
